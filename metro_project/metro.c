...
#define INF (INT_MAX/2)//防止加法溢出，Dijkstra算法必需
...
void dijkstra(Graph *g, int start, int *dist, int *prev) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        while (e != NULL) {
            int v = e->adjVex;
            if (!visited[v] && dist[u] != INF && dist[v] > dist[u] + e->weight) {
...
void shortestPathByTransfer(Graph *g, int start, int end) {
    ...
    // 临时修改权重：所有边权置1，代表一次移动，最小总步数=最少换乘+1
    EdgeNode **allEdges = malloc(g->edgeNum * sizeof(EdgeNode*));
    int edgeIdx = 0;
    for (int i = 0; i < n; i++) {
        EdgeNode *e = g->vertices[i].firstEdge;
        while (e != NULL) {
    ...
    // 恢复原始权重这里不保存原值，采用线路判断换乘次数
    int transferCount = 0;
    int cur = end;
    int preLine = -1;
    while (prev[cur] != -1) {
        int u = prev[cur];
        int curLine = g->vertices[cur].lineIds[0];
        int uLine = g->vertices[u].lineIds[0];
        if (curLine != uLine) {
            transferCount++;
        }
        cur = u;
    ...
    if (dist[end] == INF) {
        ...
        printf("最少换乘路径 (总共换乘 %d 次)：", transferCount);
        printPath(g, prev, start, end);
        printf("\n");
    }
